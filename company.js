/**
 * Company Module - Company Validation and Data Management for GARMIN CLUJ SRL
 */
...
export async function getCompanyData() {
  ...
  if (!cachedData?.summary?.cif) {
    ...
    const exactMatch = searchResults.find(c => 
      c.name.toUpperCase().startsWith("GARMIN CLUJ") ||
      c.name.toUpperCase().includes("GARMIN CLUJ")
    );
    
    let selectedCIF;
    if (!exactMatch) {
      console.log("No exact match, trying first active company...");
